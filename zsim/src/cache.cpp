...
void Cache::set_priority(Address addr, bool priority) {
    bool in_l1i = name.find("l1i") != std::string::npos;
    if(in_l1i) {addr = procMask | (addr >> lineBits);}
    array->set_priority(addr, priority);
}
...
uint64_t Cache::access(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        if (lineId == -1 && cc->shouldAllocate(req)) {
            ...
            cc->processEviction(req, wbLineAddr, lineId, respCycle, array->return_priority(wbLineAddr)); //1. if needed, send invalidates/downgrades to lower level
