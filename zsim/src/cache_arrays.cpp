...
void SetAssocArray::set_priority(Address addr, bool priority){
    uint32_t set = hf->hash(0, addr) & setMask;
    uint32_t first = set*assoc;
    for (uint32_t id = first; id < first + assoc; id++) {
        if (array[id] ==  addr) {
            rp->set_priority(id, priority);
	    break;
        }
    }
}
bool SetAssocArray::return_priority(Address addr){
    uint32_t set = hf->hash(0, addr) & setMask;
    uint32_t first = set*assoc;
    for (uint32_t id = first; id < first + assoc; id++) {
        if (array[id] ==  addr) {
            return rp->return_priority(id);
        }
    }
    return 0;
}
...
void ZArray::set_priority(Address addr, bool priority) {}
bool ZArray::return_priority(Address addr) {return 0;}
