...
class CacheArray : public GlobAlloc {
	...
	virtual void set_priority(Address addr, bool priority) = 0;
	virtual bool return_priority(Address addr) = 0;
};
...
class SetAssocArray : public CacheArray {
	...
	void set_priority(Address addr, bool priority);
	bool return_priority(Address addr); //
};
