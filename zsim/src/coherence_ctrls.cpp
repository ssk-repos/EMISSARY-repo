...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId, bool priority) {
    ...
    switch (*state) {
        ...
        case E:
            {
                parents[getParentId(wbLineAddr)]->set_priority(wbLineAddr, priority);
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, g_string ll_name) {
    uint64_t respCycle = cycle;
    g_string i_s="i";
    g_string d_s="d";
    size_t found;
    ...
    switch (type) {
        ...
        case GETS:
            if (*state == I) {
                ...
                MemReq req = {lineAddr, GETS, selfId, state, cycle, &ccLock, *state, srcId, flags, name};
                ...
                profGETSMiss.inc();
		found = ll_name.find(i_s);
		if(found!=std::string::npos) {
                    profGETSMiss_I.inc();
		}
		found = ll_name.find(d_s);
		if(found!=std::string::npos) {
                    profGETSMiss_D.inc();
		}
        ...
        case GETX:
		    ...
		    found = ll_name.find(i_s);
		    if(found!=std::string::npos) {
                        profGETXMissIM_I.inc();
		    }
		    found = ll_name.find(d_s);
		    if(found!=std::string::npos) {
                        profGETXMissIM_D.inc();
		    }
		}
                else {
		    profGETXMissSM.inc();
		    found = ll_name.find(i_s);
		    if(found!=std::string::npos) {
                        profGETXMissSM_I.inc();
		    }
		    found = ll_name.find(d_s);
		    if(found!=std::string::npos) {
                        profGETXMissSM_D.inc();
		    }
		}
                uint32_t parentId = getParentId(lineAddr);
                MemReq req = {lineAddr, GETX, selfId, state, cycle, &ccLock, *state, srcId, flags, name};
                uint32_t nextLevelLat = parents[parentId]->access(req) - cycle;
                uint32_t netLat = parentRTTs[parentId];
                profGETNextLevelLat.inc(nextLevelLat);
                profGETNetLat.inc(netLat);
