...
class CC : public GlobAlloc {
    public:
        ...
        virtual uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle, bool priority) = 0; //called iff shouldAllocate returns true
...
class MESIBottomCC : public GlobAlloc {
	...
	Counter profGETSMiss_I, profGETXMissIM_I, profGETXMissSM_I; 
	Counter profGETSMiss_D, profGETXMissIM_D, profGETXMissSM_D; 
    ...
    public:
        MESIBottomCC(uint32_t _numLines, uint32_t _selfId, bool _nonInclusiveHack, g_string& _name) : numLines(_numLines), selfId(_selfId), nonInclusiveHack(_nonInclusiveHack) , name(_name){
        ...
        void initStats(AggregateStat* parentStat) {
            ...
            profGETSMiss_I.init("mGETS_I", "GETS Instruction misses");
            profGETSMiss_D.init("mGETS_D", "GETS data misses");
            profGETXMissIM.init("mGETXIM", "GETX I->M misses");
            profGETXMissIM_I.init("mGETXIM_I", "GETX I->M Instruction misses");
            profGETXMissIM_D.init("mGETXIM_D", "GETX I->M Data misses");
            profGETXMissSM.init("mGETXSM", "GETX S->M misses (upgrade misses)");
            profGETXMissSM_I.init("mGETXSM_I", "GETX S->M Instruction misses (upgrade misses)");
            profGETXMissSM_D.init("mGETXSM_D", "GETX S->M Data misses (upgrade misses)");
            ...
            parentStat->append(&profGETSMiss_I);
            parentStat->append(&profGETSMiss_D);
            parentStat->append(&profGETXMissIM);
            parentStat->append(&profGETXMissIM_I);
            parentStat->append(&profGETXMissIM_D);
            parentStat->append(&profGETXMissSM);
            parentStat->append(&profGETXMissSM_I);
            parentStat->append(&profGETXMissSM_D);
        ...
        uint64_t processEviction(Address wbLineAddr, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId, bool priority);

        uint64_t processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, g_string ll_name);
...
class MESICC : public CC {
    ...
    public:
        ...
        void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network) {
            bcc = new MESIBottomCC(numLines, childId, nonInclusiveHack, name);
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle, bool priority) {
            ...
            evCycle = bcc->processEviction(wbLineAddr, lineId, lowerLevelWriteback, evCycle, triggerReq.srcId, priority); //2. if needed, write back line to upper level
            return evCycle;
        }
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, uint64_t* getDoneCycle = nullptr) {
            ...
            if (lineId == -1 || (((req.type == PUTS) || (req.type == PUTX)) && !bcc->isValid(lineId))) { //can only be a non-inclusive wback
                ...
                respCycle = bcc->processAccess(req.lineAddr, lineId, req.type, startCycle, req.srcId, flags, req.ll_name);
...
class MESITerminalCC : public CC {
    ...
    public:
        ...
        void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network) {
            bcc = new MESIBottomCC(numLines, childId, false /*inclusive*/, name);
            bcc->init(parents, network, name.c_str());
        }
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle, bool priority) {
            bool lowerLevelWriteback = false;
            uint64_t endCycle = bcc->processEviction(wbLineAddr, lineId, lowerLevelWriteback, startCycle, triggerReq.srcId, priority); //2. if needed, write back line to upper level
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle,  uint64_t* getDoneCycle = nullptr) {
            ...
            assert(!getDoneCycle);
            //if needed, fetch line or upgrade miss from upper level
            uint64_t respCycle = bcc->processAccess(req.lineAddr, lineId, req.type, startCycle, req.srcId, req.flags, req.ll_name);
            //at this point, the line is in a good state w.r.t. upper levels
            return respCycle;
        }
