...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    } else if (replType == "EMISSARY") {
	bool sharersAware = (replType == "EMISSARY") && !isTerminal;
	// Threshold (N) decides maximum number of high priority lines that are retained at any given time 
        uint32_t hpThreshold = config.get<uint32_t>(prefix + "repl.hpThreshold", 8);
	// Random probability for pseudo-random selection for eviction
	uint32_t randProb = config.get<uint32_t>(prefix + "repl.randProb", 16);
	if (sharersAware) {
            rp = new EmissaryReplPolicy<true>(numLines, hpThreshold, randProb);
        } else {
            rp = new EmissaryReplPolicy<false>(numLines, hpThreshold, randProb);
        }
	...
	if (name.find("l1i") != std::string::npos) {
                zinfo->l1i_lat = accLat;
        }
	if (name.find("l1d") != std::string::npos) {
                zinfo->l1d_lat = accLat;
        }
    }
...
static void InitSystem(Config& config) {
    ...
    if (!zinfo->traceDriven) {
        ...
        for (const char* group : coreGroupNames) {
			...
			ocore->l2_lat = zinfo->l2_lat;
			ocore->l3_lat = zinfo->l3_lat;
			ocore->l1i_lat = zinfo->l1i_lat;
			ocore->l1d_lat = zinfo->l1d_lat;
